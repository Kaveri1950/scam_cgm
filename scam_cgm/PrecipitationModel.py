...
####
    def precipitation_entropy_profile(self, r):
        T_phi = self.Tphi(r)  # energy in erg
        temp_K = (2 * T_phi / kb).to('K')  # convert energy to temperature in Kelvin
        Z = self.metallicity.Z(r)
        LAMBDA = self.cooling.LAMBDA(temp_K, Z)  # cooling function returns erg cm^3 / s
        entropy = (2 * mu * mp * (self.tcool_tff * LAMBDA * r / 3) ** 2) ** (1 / 3)
        return entropy.to('cm**2*erg')
####
